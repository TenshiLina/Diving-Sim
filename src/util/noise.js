// Seeded random + 3D simplex noise used by the procedural asset generators.
// Everything is deterministic for a given seed so assets are reproducible.
...
export class Rng {
  ...
  // Roughly normal distribution (sum of uniforms).
  gauss(mean = 0, sd = 1) {
    return mean + sd * (this.next() + this.next() + this.next() - 1.5) * 1.414;
  }
}
